...
int main(int argc,char *argv[])
{
  if (argc != 2)  
  {
    printf("Using:/htidc/htidc/bin/synctable_increment1 xmlbuffer\n");
    printf("Example:/htidc/htidc/bin/procctl 30 /htidc/htidc/bin/synctable_increment1 \"<logfilename>/log/szqx/synctable_increment1_LOCALOBTDATA.log</logfilename><charset>Simplified Chinese_China.ZHS16GBK</charset><connstr>szidc/pwdidc@SZQX_10.153.98.24</connstr><srctname>T_LOCALOBTDATA@SZIDC</srctname><srcpkfieldname>keyid</srcpkfieldname><dstpkfieldname>keyid</dstpkfieldname><srcfields></srcfields><dsttname>T_LOCALOBTDATA</dsttname><dstfields></dstfields><timetvl>10</timetvl><and></and><ignoreerror>FALSE</ignoreerror><maxcount>300</maxcount><month>01</month><alarmbz>FALSE</alarmbz>\"\n\n");

    printf("这是一个工具程序，用于在Oracle数据库之间同步数据，同步的方法增量，不能同步修改和删除操作。\n");
    printf("本程序不需要在源数据库中创建触发器和日志表，但是，要求表的主键字段一定是整数，并且为自增型字段。\n");
    printf("<logfilename>/log/szqx/synctable_increment1_LOCALOBTDATA.log</logfilename> 本程序运行的日志文件名。\n");
    ...
    printf("<charset>Simplified Chinese_China.ZHS16GBK</charset> 数据库的字符集，必须要与<connstr>"\
           "参数描述的数据库相同。\n");
    printf("<srctname>T_LOCALOBTDATA@SZIDC</srctname> 数据源表名，在表名后加数据库链路参数，就可以指向远程数据库。\n");
  ...
  // 注意，程序超时是1200秒
  ProgramActive.SetProgramInfo(&logfile,"synctable_increment1",1200);
...
BOOL CheckDstTName()
{
  ...
  sqlstatement stmt;
  stmt.connect(&conn);
  stmt.prepare("select column_name from USER_IND_COLUMNS where table_name=upper(:1) and column_name=upper('%s')",strdstpkfieldname);
...
void CallQuit(int sig)
{
  ...
  logfile.Write("synctable_increment1 exit.\n");
...
BOOL SyncTable()
{
  ...
  insstmt.prepare("insert into %s(%s) select %s from %s where %s in (%s)",strdsttname,strdstfields,strsrcfields,strsrctname,strsrcpkfieldname,strBindStr);
  /*
  // 由于不同步修改和删除操作，所以不启用以下代码
  insstmt.prepare("\
    BEGIN\
      delete from %s where %s in (%s);\
      insert into %s(%s) select %s from %s where %s in (%s) %s;\
    END;",strdsttname,strdstpkfieldname,strBindStr,strdsttname,strdstfields,strsrcfields,strsrctname,strsrcpkfieldname,strBindStr,strand);\
  */
  for (ii=0; ii<uMaxCount; ii++)
  {
  ...
  {
    insstmt.prepare("insert into %s(%s) select %s from %s where %s=:1 %s",strdsttname,strdstfields,strsrcfields,strsrctname,strdstpkfieldname,strand);
    ...
    insstmt.prepare("\
      BEGIN\
        ...
        insert into %s(%s) select %s from %s where %s=:2 %s;\
      END;",strdsttname,strdstpkfieldname,strdsttname,strdstfields,strsrcfields,strsrctname,strdstpkfieldname,strand);
    insstmt.bindin(1,&keyidn[ii]);
