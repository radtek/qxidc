...
vector<struct st_pallet> vpallet;

// 从输入的参数文件中加载调色板到vpallet容器中
BOOL LoadPallet();
...
int main(int argc,char *argv[])
{
  ...
  {
    printf("\n");
    printf("Using:/htidc/htidc/bin/drawcontour logfilename xmlbuffer\n\n");

    printf("Examp:/htidc/htidc/bin/drawcontour /tmp/drawcontour.log \""\
                  ...
                  "<poli>on</poli>"\
                  "<corp>618x618+80+0</corp>"\
                  "<mapfilename>/tmp/map.gif</mapfilename>"\
                  "<palletfilename>/tmp/pallet.txt</palletfilename>"\
                  "<txtfilename>/tmp/temp.txt</txtfilename>"\
                  "<giffilename>/tmp/temp.gif</giffilename>"\
                  "\"\n\n");

    printf("本程序用于把站点数据画等值线图，logfilename为日志文件名，xmlbuffer为画等值线的各种参数。\n");
    printf("本程序运行时在/tmp/htidc/tmp目录下生成用于画图的临时文件，运行完成后自动删除这些临时文件。\n\n");
    ...
    printf("gxout          生成图形的种类，contour-等值线图，shaded-色斑图。\n");
    printf("dispvalue      是否显示每个真实站点的数值，true-显示，false-不显示。\n");
    ...
    printf("palletfilename 调色板数据文件名，调色板数据文件CSV文件，每一行表示一个调色方案，"\
           "格式\"数值,色1,色2,色3\"，注意，如果站点的值在调色板的范围之外，将会被丢弃。\n");
    printf("txtfilename    站点数据文件名，站点数据文件为CSV文件，每一行表示一个站点的数据，格式\"纬度,经度,数值\"。\n");
  ...
  //打开告警
  logfile.SetAlarmOpt("drawcontour");

  // 解析xmlbuffer，读取各种参数
  if (GetXMLBuffer(argv[2]) == FALSE) EXIT(-1);

  // 从输入的参数文件中加载调色板到vpallet容器中，一定要放在DataTxtToBin函数之前
  if (LoadPallet() == FALSE) EXIT(-1);
...
BOOL GetXMLBuffer(char *xmlbuffer)
{
  ...
  GetXMLBuffer(xmlbuffer,"palletfilename",palletfilename,300);
  ...
  if (strlen(palletfilename) == 0) { logfile.Write("palletfilename is null.\n"); return FALSE; }

  memset(tmpgiffilename,0,sizeof(tmpgiffilename));
  snprintf(tmpgiffilename,300,"/tmp/htidc/tmp/grads_%06d.png",getpid());
...
BOOL DataTxtToBin()
{
  ...
  while (TRUE)
  {
    ...
    if ( (CmdStr.m_vCmdStr[0].length()==0) || (CmdStr.m_vCmdStr[1].length()==0) || (CmdStr.m_vCmdStr[2].length()==0) ) continue;
    ...
    val=atof(CmdStr.m_vCmdStr[2].c_str());
    ...
    // 如果站点的值在调色板之外，就丢弃这个点
    if ( (val<atof(vpallet[0].value)-0.001) || (val>atof(vpallet[vpallet.size()-1].value)+0.001) ) continue;

    // 检查站点数据的合法性
    if (strlen(minvalue) > 0) { if (val<atof(minvalue)-0.001) continue; }
    if (strlen(maxvalue) > 0) { if (val>atof(maxvalue)+0.001) continue; }

    fwrite(&stdata,sizeof(struct st_stdata),1,DatFile.m_fp);

    fwrite(&val,sizeof(float),1,DatFile.m_fp);
...
BOOL CrtCtlFile()
{
  ...
  File.Fprintf("VARS 1\n");
  File.Fprintf("val 0 99 rainfall for day\n");
...
BOOL CrtGSFile()
{
  ...
  File.Fprintf("'set mpdset cnhimap'\n");
  //File.Fprintf("'set mpdset cnmap'\n");
  //File.Fprintf("'set mpdset off'\n");
  ...
  // 定义调色板
  // 设置对应于set clevs命令设定的特定等值线的颜色
  char strtmp[20],strclevs[10240],strccols[10240];
  memset(strclevs,0,sizeof(strclevs));
  memset(strccols,0,sizeof(strccols));
  for (UINT ii=20;ii<vpallet.size()+20;ii++)
  {
    File.Fprintf("'set rgb %d %s'\n",ii,vpallet[ii-20].colors);

    memset(strtmp,0,sizeof(strtmp));
    snprintf(strtmp,15," %s",vpallet[ii-20].value); strcat(strclevs,strtmp);

    memset(strtmp,0,sizeof(strtmp));
    snprintf(strtmp,15," %lu",ii); strcat(strccols,strtmp);
  }

  // 设置画图的方式
  File.Fprintf("'set gxout %s'\n",gxout);

  // 设置的区域用于等值线绘图、地图绘图、单线绘图，该区域内以虚页英寸为单位。缺省时，自动按图形类型设置绘图区域。
  File.Fprintf("'set parea 0 11 0 8.5'\n");

  // 不画低于cmin和cmax的等值线，这个控制没什么用
  // 虽然不画低于cmin和cmax的等值线，但低于cmin和cmax的站点值会影响插值结果
  // File.Fprintf("'set cmin %s'\n",vpallet[0].value);
  // File.Fprintf("'set cmax %s'\n",vpallet[vpallet.size()-1].value);

  // 注意，每画一次图，就要重新设置一次色板
  File.Fprintf("'set clevs %s'\n",strclevs);
  File.Fprintf("'set ccols %s'\n",strccols);

  // 画图
  File.Fprintf("'define aa=oacres(stgrib,val.2,16,8,6,4,3,2,1)'\n");
  File.Fprintf("'d aa'\n");
  ...
  File.Fprintf("'set gxout contour'\n");
  File.Fprintf("'d aa'\n");
  */
...
BOOL LoadPallet()
{
  ...
  if (File.OpenForRead(palletfilename,"r") == FALSE)
  {
    logfile.Write("Fopen %s failed.\n",palletfilename); return FALSE; 
  }
  ...
  while (TRUE)
  {
    ...
    memset(&stpallet,0,sizeof(stpallet));

    if (File.FFGETS(strLine,100)==FALSE) break;

    CmdStr.SplitToCmd(strLine,",");

    if (CmdStr.CmdCount() != 4) continue;
    
    strncpy(stpallet.value,CmdStr.m_vCmdStr[0].c_str(),10);
    snprintf(stpallet.colors,20,"%3s %3s %3s",CmdStr.m_vCmdStr[1].c_str(),CmdStr.m_vCmdStr[2].c_str(),CmdStr.m_vCmdStr[3].c_str());

    vpallet.push_back(stpallet);

    // 最多允许1000个调色板
    if (vpallet.size()>=1000) break;
  }
  ...
  if (vpallet.size() == 0)
  {
    logfile.Write("load %s is failed.\n",palletfilename); return FALSE;
  }
...
/*
# contour shaded
/htidc/htidc/bin/drawcontour /tmp/drawcontour.log "<gradsroot>/htidc/grads-2.0.a9</gradsroot><lonstart>111.80</lonstart><lonstep>0.05</lonstep><lonsum>33</lonsum><latstart>21.53</latstart><latstep>0.05</latstep><latsum>27</latsum><gxout>contour</gxout><dispvalue>false</dispvalue><minvalue>10</minvalue><maxvalue>35</maxvalue><poli>on</poli><corp>618x618+90+0</corp><mapfilename></mapfilename><palletfilename>/tmp/temppallet.txt</palletfilename><txtfilename>/tmp/temp_all.txt</txtfilename><giffilename>/tmp/temp.gif</giffilename>"
*/
