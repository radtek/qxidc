...
int main(int argc,char *argv[])
{
  if (argc != 2)  
  {
    printf("Using:/htidc/htidc/bin/synctable_update1 xmlbuffer\n");
    printf("Example:/htidc/htidc/bin/procctl 50 /htidc/htidc/bin/synctable_update1 \"<logfilename>/log/szqx/synctable_update1_INDEXINFO.log</logfilename><charset>Simplified Chinese_China.ZHS16GBK</charset><connstr>szidc/pwdidc@SZQX_10.153.98.24</connstr><srctname>T_INDEXINFO@SZIDC</srctname><srcpkfieldname>cityid</srcpkfieldname><dstpkfieldname>cityid</dstpkfieldname><srcfields>cityid,cityname,pubdate,gm</srcfields><dsttname>T_INDEXINFO</dsttname><dstfields>cityid,cityname,pubdate,gm</dstfields><where>where pubdate>sysdate-1</where><maxcount>300</maxcount><alarmbz>FALSE</alarmbz>\"\n\n");
    ...
    printf("还有，如果源表没有delete操作，用synctable_update1和synctable_update2都可以，如果有delete操作，就只能用synctable_update2了。\n");
    printf("<logfilename>/log/szqx/synctable_update1_INDEXINFO.log</logfilename> 本程序运行的日志文件名。\n");
    printf("<connstr>szidc/pwdidc@SZQX_10.153.98.24</connstr> 目的数据库的连接参数。\n");
  ...
  // 注意，程序超时是1200秒
  ProgramActive.SetProgramInfo(&logfile,"synctable_update1",1200);
...
void CallQuit(int sig)
{
  ...
  logfile.Write("synctable_update1 exit.\n");
