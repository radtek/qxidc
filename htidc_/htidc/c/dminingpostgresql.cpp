...
int main(int argc,char *argv[])
{
  if (argc != 3)
  {
    ...
    printf("增量采集标志字段的值存放的文件 <incfilename>/tmp/htidc/list/dminingpostgresql_OBTHOURD_HYCZ.list</incfilename> 每次采集完数据后，incfield的最大值将存放在此文件中。\n");
  ...
  //打开告警
  logfile.SetAlarmOpt("dminingpostgresql");
  ...
  logfile.Write("dminingpostgresql beginning.\n");

  // 注意，程序超时是1200秒
  ProgramActive.SetProgramInfo(&logfile,"dminingpostgresql",1200);
...
void CallQuit(int sig)
{
  ...
  logfile.Write("dminingpostgresql exit.\n");
...
/*
/htidc/htidc/bin/dminingpostgresql /tmp/htidc/log/dminingpostgresql_SEND_LIST_HIS.log "<charset>Simplified Chinese_China.ZHS16GBK</charset><connstr>hzzwt/esaserver</connstr><firstsql>truncate table SEND_LIST_HIS</firstsql><selectsql>select clientserialno,to_char(crttime,'yyyy-mm-dd hh24:mi:ss'),sendno from SEND_LIST_HIS where crttime>sysdate-5</selectsql><fieldstr>clientserialno,crttime,sendno</fieldstr><fieldlen> </fieldlen><bfilename>SEND_LIST_HIS</bfilename><efilename>HZZWT</efilename><outpathtmp>/tmp/htidc/tmp</outpathtmp><outpath>/tmp/htidc/ftpput</outpath><starttime></starttime><endsql></endsql><incfield>sendno</incfield><incfilename>/tmp/htidc/list/dminingpostgresql_SEND_LIST_HIS.list</incfilename>"
*/
