...
int ftplib::readresp(char c, ftphandle *nControl)
{
    ...
    if (m_timeout == 1) return 0;
    
    if (readline(nControl->response,256,nControl) == -1) return 0;
...
int ftplib::Connect(const char *host)
{
    ...
    if (pnum == NULL)
    {
        if ((pse = getservbyname("ftp","tcp")) == NULL) return 0;
    ...
    {
        if ((phe = gethostbyname(lhost)) == NULL) return 0;
    ...
    if (sControl <= 0) return 0;
    
    if (setsockopt(sControl,SOL_SOCKET,SO_REUSEADDR, SETSOCKOPT_OPTVAL_TYPE &on, sizeof(on)) == -1)
    {
        net_close(sControl); return 0;
    }

    int iret=connect(sControl, (struct sockaddr *)&sin, sizeof(sin));
...
int ftplib::FtpSendCmd(const char *cmd, char expresp, ftphandle *nControl)
{
    ...
    memset(nControl->response,0,sizeof(nControl->response));
    ...
    if (x <= 0)
    {
      sprintf(nControl->response,"send %s Failed.",buf); return 0; 
    }
...
int ftplib::FtpXfer(const char *localfile, const char *path, ftphandle *nControl, accesstype type, transfermode mode)
{
    ...
    if (localfile != NULL)
    {
        ...
        if (local == NULL)
        {
            strncpy(nControl->response, strerror(errno), sizeof(nControl->response)); return 0;
        }
    }
    ...
    { 
      ...
      if ( (type == ftplib::dir) && (strncmp(nControl->response,"550",3) == 0) ) return 1;
...
int ftplib::Get(const char *outputfile, const char *path, transfermode mode, long offset)
{
...
int ftplib::Put(const char *inputfile, const char *path, transfermode mode, long offset)
{
    mp_ftphandle->offset = offset;
...
int ftplib::Quit()
{
    ...
    {
        net_close(mp_ftphandle->handle); mp_ftphandle->handle=0;
        return 0;
    }
    else
    {
...
void ftplib::ClearHandle()
{
    mp_ftphandle->dir = FTPLIB_CONTROL;
    mp_ftphandle->ctrl = NULL;
    // 在该程序中最初是把模式设为ftplib::pasv，但听说有些ftpserver不支持这种模式，金安的网闸也不支持
    // 所以把缺省模式改为ftplib::port，目前还不知道用ftplib::port模式传输文件会不会有什么问题
