...
BOOL Cftp::login(const char *in_ip,const int in_port,const char *in_username,const char *in_password,const int in_mode)
{
  if (m_ftpconn != 0) { FtpQuit(m_ftpconn); m_ftpconn=0; }

  m_connectfailed=m_loginfailed=m_optionfailed=FALSE;

  if (FtpConnect(in_ip,in_port,&m_ftpconn) == FALSE)  { m_connectfailed=TRUE; return FALSE; }
...
BOOL Cftp::get(const char *in_remotefilename,const char *in_localfilename,const BOOL bCheckSize)
{
  ...
  if (RENAME(strlocalfilenametmp,in_localfilename) == FALSE) return FALSE; 
