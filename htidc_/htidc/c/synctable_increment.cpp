...
int main(int argc,char *argv[])
{
  if (argc != 2)  
  {
    ...
    printf("这是一个工具程序，用于在Oracle数据库之间同步数据，同步的方法增量，可同步增加、修改和删除三种操作。\n");
    printf("如果数据源表有自增字段，可采用synctable_increment1程序，它不需要在源数据库中创建触发器和日志表。\n");
    printf("<logfilename>/log/sqxj/synctable_increment_ALLAWSDATA.log</logfilename> 本程序运行的日志文件名。\n");
    ...
    printf("  4、如果<timetvl>不超过30秒，本程序常驻内存，如果超过了30秒（包括30秒），执行完一次同步后将退出。\n\n");
    printf("  5、<maxcount>参数非常重要，如果数据源表有非法数据，当<maxcount>为1时，失败的记录无法同步，但不\n");
