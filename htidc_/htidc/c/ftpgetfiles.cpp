...
int main(int argc,char *argv[])
{
  if (argc != 3)
  {
    ...
    printf("<indexid>10001</indexid> 数据监控的流程编号，如果为空，则不生成数据采集监控文件。\n");
  ...
  ProcMatchDTime(strremotepathbak);
  */
  ...
  logfile.Write("remotepathbak=%s\n",strremotepathbak);
  logfile.Write("matchname=%s\n",strmatchname);
  logfile.Write("listfilename=%s\n",strlistfilename);
  logfile.Write("deleteremote=%lu\n",udeleteremote);
  logfile.Write("timeout=%lu\n",utimeout);
  logfile.Write("mtime=%lu\n",umtime);
  */
  ...
  // 写进程活动日志文件
  RealMon.WriteToProActLog(strindexid,"ftpgetfiles",utimeout);
  ...
  for (UINT ii=0;ii<m_vrfilelist.size();ii++)
  {
    ...
    if (m_vrfilelist[ii].ftpsts==1)
    {
      ...
      if (ftpgetfiles() == TRUE) 
      {
        ...
        {
          memset(&RealMon.m_stCOLLECTLOG,0,sizeof(RealMon.m_stCOLLECTLOG));

          strcpy(RealMon.m_stCOLLECTLOG.indexid,strindexid);
	  getLocalIP(RealMon.m_stCOLLECTLOG.serverip);
	  strcpy(RealMon.m_stCOLLECTLOG.programname,"ftpgetfiles");
	  strcpy(RealMon.m_stCOLLECTLOG.colltype,"ftp");
	  strcpy(RealMon.m_stCOLLECTLOG.remoteip,strremoteip);
	  strcpy(RealMon.m_stCOLLECTLOG.filename,stfilelist.localfilename);
	  snprintf(RealMon.m_stCOLLECTLOG.filesize,50,"%d",stfilelist.filesize);
	  strcpy(RealMon.m_stCOLLECTLOG.filetime,stfilelist.modtime);

	  memset(strLocalTime,0,sizeof(strLocalTime));
	  LocalTime(strLocalTime,"yyyymmddhh24miss");
	  strcpy(RealMon.m_stCOLLECTLOG.colltime,strLocalTime);

	  RealMon.m_vCOLLECTLOG.push_back(RealMon.m_stCOLLECTLOG);
        }
      }
    }
  }
  ...
  // 生成数据采集日志XML文件
  if (RealMon.WriteToCollectLog() == FALSE) logfile.Write("WriteToCollectLog failed.\n");
...
BOOL ftpgetfiles()
{
  ...
  logfile.WriteEx("ok(size=%d,time=%d,modtime=%s).\n",stfilelist.filesize,Timer.Elapsed(),stfilelist.modtime); 
...
BOOL WriteToXML()
{
  ...
  // 把已采集成功的文件信息文件从m_vrfilelist中写入本地清单文件
  ...
  for (UINT ii=0;ii<m_vrfilelist.size();ii++)
  {
    if (m_vrfilelist[ii].ftpsts==2)
    {
      fprintf(listfp,"<filename>%s</filename><modtime>%s</modtime><filesize>%d</filesize><endl/>\n",m_vrfilelist[ii].remotefilename,m_vrfilelist[ii].modtime,m_vrfilelist[ii].filesize);
    }
  }
